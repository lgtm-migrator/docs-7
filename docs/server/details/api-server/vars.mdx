...
export const DatabaseCaution = () => (
  <Admonition type="caution">
    <p>
      ...
      <a href="#SQLITE">SQLITE</a>のうち、少なくとも 1
      つの値を設定する必要があります。
    </p>
  </Admonition>
);
...
bcrypt のハッシュ値は様々な方法で生成できます。どの方法を用いても構いません。Flocon では次の2つの方法を提供しています。

- [https://tools.flocon.app/bcrypt](https://tools.flocon.app/bcrypt) で生成する。最も手軽ですが、このページには [npm の bcryptjs パッケージ](https://www.npmjs.com/package/bcryptjs) を使っているため、APIサーバーに使用している [bcrypt パッケージ](https://www.npmjs.com/package/bcrypt)では認識できないハッシュ値が生成される可能性があります。その場合は、日本語などを用いない、もしくはパスワードを短くすることで解決できる可能性があります。
- [Floconのソースコード](https://github.com/flocon-trpg/servers)に付属している`bcrypt-interactive`スクリプトを利用する。`apps/api-server`ディレクトリで `yarn run bcrypt-interactive`を実行すると、bcrypt ハッシュ値生成プログラムが起動します。[bcrypt パッケージ](https://www.npmjs.com/package/bcrypt) が使われますが、事前に Node.js と yarn のインストール、`yarn install` コマンドによる数百MBのファイルのインストールをする必要があるなど、ハードルが高いのが欠点です。
